...
const RevenueChart = ({ projects = [] }) => {
  // if some projects are provided, derive data from them, otherwise fall back to dummy
  const data = projects.length > 0 ?
    projects.map(p => {
      const projectName = p.projectId || p.id || "";
      const agreement = parseFloat(p.agreement?.agreementSum) || 0;
      const expenses = parseFloat(p.totalExpense) || 0;
      const revenue = agreement - expenses;
      return { project: projectName, revenue, expenses };
    }) :
    [
