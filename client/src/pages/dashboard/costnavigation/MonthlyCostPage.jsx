...
const MonthlyCostPage = () => {
  ...
  const preparePieChartData = (type) => {
    ...
    allCostData.forEach(record => {
      // Monthly data structure uses different field names
  ...
  useEffect(() => {
    fetchMonthlyCosts();
  }, [year, month]); // Add year and month dependencies to automatically fetch when they change
  ...
  const fetchMonthlyCosts = async () => {
    try {
      ...
      const response = await dashboardAPI.getMonthlyCosts(year, month);
      ...
      setError("Failed to fetch monthly cost data. Please try again.");
      setCostData([]);
      setAllCostData([]);
      setTotalPages(1);
      toast({
        title: "Error",
        description: "Could not load monthly cost data.",
        status: "error",
        duration: 3000,
        isClosable: true,
      });
  ...
  const handleDateChange = ({ year: newYear, month: newMonth }) => {
  ...
  return (
    <Container maxW="container.xl" py={6}>
      ...
      {/* Page Header */}
      <Heading size="lg" mb={2} color="cyan.600">Monthly Cost Overview</Heading>
      <Text color="gray.500" mb={4}>
        View cost breakdown for {currentMonthName} {year} ({period.startDate} to {period.endDate})
      </Text>
      ...
      <SimpleGrid columns={{ base: 1, md: 3 }} spacing={6} mb={4}>
        <SummaryCard 
          title={`${currentMonthName} Labor Cost`} 
        ...
        <SummaryCard 
          title={`${currentMonthName} Material Cost`} 
        ...
        <SummaryCard 
          title={`${currentMonthName} Total Cost`} 
      ...
      {allCostData.length > 0 && !isLoading && (
        <Box mt={8}>
          <Heading size="md" mb={2}>
            Monthly Cost Distribution by Jobs
          </Heading>
          <Text color="gray.500" mb={6}>
            Visual breakdown of how costs are distributed across different jobs for {currentMonthName} {year}
          </Text>
      ...
      {allCostData.length === 0 && !isLoading && (
        <Box mt={8} textAlign="center" py={8}>
          <Text color="gray.500">
            No cost data available for {currentMonthName} {year} to display charts.
          </Text>
        </Box>
      )}
    </Container>
