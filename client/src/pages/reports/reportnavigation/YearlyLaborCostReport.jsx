...
const YearlyLaborCostReport = () => {
  ...
  const fetchReportData = async () => {
    try {
      ...
      const response = await reportsAPI.getYearlyLaborCostReport(formattedStartDate, formattedEndDate);
  ...
  // Calculate yearly cost summary data
  const calculateYearlyCostSummary = () => {
    ...
    reportData.forEach(cost => {
      ...
      // This matches what the dashboard does: parseFloat(cost.cost) || 0
      const dailyCost = parseFloat(cost.cost) || 0;
      
      // For reports, we still want to show the breakdown by cost type
      // So we calculate the breakdown from labor assignments but ensure total matches the daily cost
  ...
  // Calculate yearly labor work summary by trade
  const calculateYearlyLaborWorkSummary = () => {
    const jobMap = new Map();
  ...
  const handleDownloadPDF = () => {
    ...
    try {
      ...
      doc.text('Yearly Labor Cost Report', pageWidth / 2, yPosition, { align: 'center' });
      ...
      doc.setFontSize(16);
      doc.setFont(undefined, 'bold');
      doc.text('Yearly Labor Work Summary (Working Hours by Trade)', 14, yPosition);
      yPosition += 10;

      const laborWorkSummaryData = calculateYearlyLaborWorkSummary();
      ...
      doc.setFont(undefined, 'normal');
      const notes = [
        '• This report summarizes labor costs for the entire year.',
      ...
      // Save the PDF
      const fileName = `Yearly_Labor_Cost_Report_${selectedYear}.pdf`;
      doc.save(fileName);
  ...
  const laborWorkSummaryData = reportData.length > 0 ? calculateYearlyLaborWorkSummary() : [];
  ...
  return (
    <Box w="100%" px={{ base: 2, md: 4, lg: 6 }} py={{ base: 4, md: 6 }}>
      <VStack spacing={{ base: 4, md: 6 }} align="stretch">
        {/* Header */}
        <div>
          <Heading size="lg" mb={2} color="teal.600">Yearly Labor Cost Report</Heading>
          <Text color="gray.500" mb={4}>
            Generate yearly labor cost reports with detailed cost and work hour breakdowns
          </Text>
        </div>
            ...
            <VStack spacing={4} align="stretch">
              ...
              <HStack spacing={4} align="end" flexWrap="wrap">
                <FormControl maxW="120px">
                  <FormLabel htmlFor="yearlyLaborCostYear" fontSize="sm">Year</FormLabel>
                  <Select
                    id="yearlyLaborCostYear"
                    name="yearlyLaborCostYear"
                    value={selectedYear}
        ...
        {loading && (
          <Center py={10}>
            <VStack>
              ...
              <Text mt={4} color="gray.500">Generating yearly report...</Text>
            </VStack>
          </Center>
        )}
        ...
        {selectedYear && reportData.length === 0 && !loading && !error && (
          <Alert status="info">
            ...
            <Box>
              ...
              <Text fontSize="sm">No labor cost data found for year {selectedYear}.</Text>
            </Box>
          </Alert>
        )}
            ...
            <CardBody>
              <VStack spacing={6} align="stretch">
                ...
                <Box textAlign="center">
                  <Heading size="lg" mb={2}>Yearly Labor Cost Report</Heading>
                  ...
                  {costSummaryData.length > 0 ? (
                    ...
                    <Center py={10}>
                      <VStack>
                        ...
                        <Text color="gray.500">No labor cost data found for this year</Text>
                      </VStack>
                    </Center>
                  )}
                </Box>
                ...
                {/* Labor Work Summary Table */}
                <Box>
                  <Heading size="md" mb={4}>Yearly Labor Work Summary (Working Hours by Trade)</Heading>
                  ...
                  {laborWorkSummaryData.length > 0 ? (
                    ...
                    <Center py={10}>
                      <VStack>
                        <Icon as={FiFileText} boxSize={12} color="gray.400" />
                        <Text color="gray.500">No labor work data found for this year</Text>
                      </VStack>
                    </Center>
                  )}
                </Box>
          ...
          <Center py={20}>
            <VStack spacing={4}>
              ...
              <Text color="gray.400" textAlign="center">
                Choose a year to generate the yearly labor cost report
              </Text>
            </VStack>
          </Center>
        )}
...
export default YearlyLaborCostReport;
