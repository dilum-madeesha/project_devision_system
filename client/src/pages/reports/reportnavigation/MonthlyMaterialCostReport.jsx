...
const MonthlyMaterialCostReport = () => {
  ...
  const fetchReportData = async () => {
    try {
      ...
      console.log(`Material Report Date Range: ${formattedStartDate} to ${formattedEndDate}`);

      // Fetch all material cost data for the date range
      const response = await reportsAPI.getMonthlyMaterialCostReport(formattedStartDate, formattedEndDate);
      ...
      console.log(`Monthly Material Report: Found ${allMaterialCosts.length} records for ${formatDate(selectedYear, selectedMonth)}`);
      ...
      console.error("Error fetching material report data:", err);
      setError(err.message || "Failed to fetch material report data");
      toast({
        title: "Error fetching data",
        description: err.message || "Failed to fetch material report data",
        status: "error",
        duration: 5000,
        isClosable: true,
      });
  ...
  // Calculate monthly material cost summary data (Table 1: Job-wise totals)
  ...
  const handleDownloadPDF = () => {
    ...
    try {
      ...
      doc.text('Monthly Material Cost Report', pageWidth / 2, yPosition, { align: 'center' });
      ...
      doc.setFont(undefined, 'normal');
      const notes = [
        '• This report summarizes material costs for the entire month.',
      ...
      // Save the PDF
      const fileName = `Monthly_Material_Cost_Report_${selectedYear}_${selectedMonth.padStart(2, '0')}.pdf`;
      doc.save(fileName);
  ...
  return (
    <Box w="100%" px={{ base: 2, md: 4, lg: 6 }} py={{ base: 4, md: 6 }}>
      <VStack spacing={{ base: 4, md: 6 }} align="stretch">
        {/* Header */}
        <div>
          <Heading size="lg" mb={2} color="teal.600">Monthly Material Cost Report</Heading>
          <Text color="gray.500" mb={4}>
            Generate monthly material cost reports with detailed order analysis
          </Text>
        </div>
        ...
        {loading && (
          <Center py={10}>
            <VStack>
              ...
              <Text mt={4} color="gray.500">Generating monthly material report...</Text>
            </VStack>
          </Center>
        )}
        ...
        {selectedYear && selectedMonth && reportData.length === 0 && !loading && !error && (
          <Alert status="info">
            ...
            <Box>
              ...
              <Text fontSize="sm">No material cost data found for {formatDate(selectedYear, selectedMonth)}.</Text>
            </Box>
          </Alert>
        )}
            ...
            <CardBody>
              <VStack spacing={6} align="stretch">
                ...
                <Box textAlign="center">
                  <Heading size="lg" mb={2}>Monthly Material Cost Report</Heading>
                ...
                {/* Table 1: Monthly Cost Summary (Job-wise totals) */}
                <Box>
                  ...
                  {costSummaryData.length > 0 ? (
                    ...
                    <Center py={10}>
                      <VStack>
                        ...
                        <Text color="gray.500">No material cost data found for this month</Text>
                      </VStack>
                    </Center>
                  )}
                </Box>
                  ...
                  {materialOrdersData.length > 0 ? (
                    ...
                    <Center py={10}>
                      <VStack>
                        ...
                        <Text color="gray.500">No material orders found for this month</Text>
                      </VStack>
                    </Center>
                  )}
                </Box>
          ...
          <Center py={20}>
            <VStack spacing={4}>
              ...
              <Text color="gray.400" textAlign="center">
                Choose a month and year to generate the monthly material cost report
              </Text>
            </VStack>
          </Center>
        )}
...
export default MonthlyMaterialCostReport;
